...
# SELECT ALL THE OFFICES ORDERED BY CITY NAME
...
#SELECT ORDER BY COUNTRY AND ORDER BY CITY
...
SELECT `officeCode`, `territory`, `country`, `state`, `city`, `addressLine1`, `addressLine1`,
`addressLine2`, `phone`
FROM `offices`
ORDER BY `territory`, `country`, `state`, `city`, `postalCode`;
...
SELECT *
FROM `products`
#WHERE`productCode`= "S10_1678" OR `productCode`= "10_1949"
WHERE `productCode`IN ("S10_1678", "10_1949");
...
#PRODUCT NAME WITH THE HIGHEST PROFIT MARGIN (`MSRP`- BUYPRICE = HIGHEST PROFIT MARGIN)
...
#SALES PERSON - WHO MADE THE BIGGEST SALE EVER - DOCUMENT DATA BASES

SELECT orderNumber, SUM((`priceEach` * `quantityOrdered`)) AS `orderPrice`
FROM `orderdetails`
